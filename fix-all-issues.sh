...
# First, let's check the backend status
echo "🌐 Checking backend status..."
BACKEND_URL="https://quantum-ai-backend.wittydune-e7dd7422.eastus.azurecontainerapps.io"
if curl -s --head "$BACKEND_URL/api/health" | grep "200" > /dev/null; then
    echo "✅ Backend is running"
else
    echo "❌ Backend is not responding (502 error)"
    echo "🔄 Redeploying backend to Azure..."
    ./deploy-azure.sh
fi

# Now let's fix the frontend canvas rendering issue
...
class QuantumSynth {
  ...
  constructor(canvas: HTMLCanvasElement, visualizationElement: HTMLElement, statusElement: HTMLElement, statusDot: HTMLElement) {
    ...
    // Observe the visualization container, not the canvas itself
  ...
  private setupCanvas() {
    // Get the container dimensions instead of the canvas
    const container = document.querySelector('.visualization-container');
    if (!container) return;
    
    const rect = container.getBoundingClientRect();
    const width = rect.width;
    const height = rect.height - 100; // Account for header and footer
