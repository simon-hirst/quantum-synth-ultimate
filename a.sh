...
[ -x "./git_smart_commit.sh" ] || { echo "❌ ./git_smart_commit.sh missing/executable"; exit 1; }

VS_FILE="frontend/src/visualizer.ts"
CSS_FILE="frontend/src/style.css"

[ -f "$VS_FILE" ] || { echo "❌ $VS_FILE not found"; exit 1; }
[ -f "$CSS_FILE" ] || { echo "❌ $CSS_FILE not found"; exit 1; }

ts(){ date +%s; }

echo "🧩 Backing up files…"
cp -f "$VS_FILE" "$VS_FILE.$(ts).bak"
cp -f "$CSS_FILE" "$CSS_FILE.$(ts).bak"

echo "🎨 Patching CSS for full-bleed container & canvas…"
cat >> "$CSS_FILE" <<'CSS'

/* ===== QuantumSynth scaling fix (full-bleed) ===== */
html, body, #app { height: 100%; margin: 0; padding: 0; }
.quantum-container {
  position: relative;
  width: 100%;
  height: 100vh; /* fill viewport; if embedded, parent can override via height */
  display: flex;
  align-items: stretch;
  justify-content: stretch;
  background: radial-gradient(1200px 800px at 50% 50%, rgba(10,14,22,0.9), rgba(3,5,9,1) 60%);
}
.quantum-container canvas#visualizer {
  display: block;
  width: 100%;
  height: 100%;
  touch-action: none;
...
echo "🛠️ Rewriting $VS_FILE with aspect-correct shaders & ResizeObserver…"
cat > "$VS_FILE" <<'TS'
/**
...
echo "📝 Smart-dated commit…"
./git_smart_commit.sh "fix(frontend): enforce full-bleed container scaling; aspect-correct all shaders with uRes; ResizeObserver-driven canvas sizing; proper viewport updates" \
  . || true

# Optional deploy if your helper exists
if [ -x "./deploy_to_aca.sh" ]; then
  echo "🚀 Deploying to ACA…"
  ./deploy_to_aca.sh
  if [ -x "./smoke_endpoints.sh" ]; then ./smoke_endpoints.sh || true; fi
fi

echo "✅ Scaling & aspect fixes applied. All visualizers should fill their container without distortion."
