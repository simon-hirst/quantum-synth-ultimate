#!/usr/bin/env bash
# Patch 002 (fixed): Replace legacy scenes with beautiful, music-reactive presets
# and remove the broken sed line that caused “unterminated `s' command”.
#
# Usage (run from repo root):
#   bash a.sh
#
# What this does:
#  - Overwrites frontend/src/visualizer.ts with 5 tasteful audio-reactive scenes:
#      auroraFlow, liquidSpectrum, neonParticles, ribbonWaves, glassCells
#  - Adds tiny helpers to frontend/src/backend-config.ts (httpBase, wsUrl)
#    so visualizer can talk to your backend without import errors.
#  - Updates the sidebar hint in frontend/src/main.ts WITHOUT using sed.
#  - Commits via scripts/qs_commit.sh (keeps your commit-dating rule).
...
# ---------------------------------------------------------------------------
# Backups (first time only)
# ---------------------------------------------------------------------------
[[ -f frontend/src/visualizer.ts && ! -f frontend/src/visualizer.ts.bak.qs002 ]] && \
  cp frontend/src/visualizer.ts frontend/src/visualizer.ts.bak.qs002 || true
[[ -f frontend/src/backend-config.ts && ! -f frontend/src/backend-config.ts.bak.qs002 ]] && \
  cp frontend/src/backend-config.ts frontend/src/backend-config.ts.bak.qs002 || true
[[ -f frontend/src/main.ts && ! -f frontend/src/main.ts.bak.qs002 ]] && \
  cp frontend/src/main.ts frontend/src/main.ts.bak.qs002 || true

# ---------------------------------------------------------------------------
# 1) Write new visualizer with the beautiful scenes
# ---------------------------------------------------------------------------
cat > frontend/src/visualizer.ts <<'TS'
/**
...
# ---------------------------------------------------------------------------
# 2) Ensure backend-config has helpers httpBase() & wsUrl()
# ---------------------------------------------------------------------------
node <<'NODE'
const fs = require('fs');
const p = 'frontend/src/backend-config.ts';
let s = fs.readFileSync(p, 'utf8');

if (!/export function httpBase/.test(s)) {
  s += `

export function httpBase(): string {
  const host = (import.meta as any).env?.VITE_BACKEND_HOST || (BACKEND_CONFIG as any).url || window.location.host;
  const secure = (BACKEND_CONFIG as any).useSecure ?? (window.location.protocol === 'https:');
...
`;
  ...
  console.log('✔ backend-config helpers added');
} else {
  console.log('• backend-config helpers already present');
}
NODE

# ---------------------------------------------------------------------------
# 3) Update sidebar hint in main.ts — NO sed here
# ---------------------------------------------------------------------------
...
fs.writeFileSync(p, s);
console.log('✔ main.ts hint updated');
NODE
fi

# ---------------------------------------------------------------------------
# 4) Stage & commit (dates handled by your qs_commit.sh)
# ---------------------------------------------------------------------------
git add frontend/src/visualizer.ts frontend/src/backend-config.ts frontend/src/main.ts
scripts/qs_commit.sh "feat(viz): replace ugly presets with 5 elegant, music-reactive scenes; add backend URL helpers; remove bad sed" --all

echo "✅ Patch applied. Keys: M next • 1–5 choose • P pause • S server (one-off)."
