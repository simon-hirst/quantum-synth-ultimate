#!/usr/bin/env bash
# scripts/patch_frontend_004_plasma_orb.sh
# Adds a new, highly music-reactive scene “plasmaOrb” (the glowing orb with
# wavy edges) and includes it in rotation. The orb breathes with bass, ripples
# on beats, and its edge vibrates with spectrum + waveform. Keeps morph
# transition and all other scenes. Also turns up the overall response multipliers.
#
# Usage:
#   bash scripts/patch_frontend_004_plasma_orb.sh
#
# Requires: scripts/qs_commit.sh (your commit dating rule)
...
# Backup once
if [[ -f frontend/src/visualizer.ts && ! -f frontend/src/visualizer.ts.bak.qs004 ]]; then
  cp frontend/src/visualizer.ts frontend/src/visualizer.ts.bak.qs004
fi

# Overwrite visualizer with plasmaOrb added + stronger reactivity
...
git add frontend/src/visualizer.ts
scripts/qs_commit.sh "feat(viz): add highly music-reactive PlasmaOrb scene; boost AGC/impact multipliers; keep morph transitions intact" --all

echo "✅ PlasmaOrb added, reactivity boosted, and changes committed.
Keys: M next • 1–0 choose • P pause • S server (preview)."
