#!/usr/bin/env bash
# scripts/patch_commit_script_fix.sh
# Replaces scripts/qs_commit.sh with a robust, fair 50/50 dating rule:
# - Each commit date is EITHER the same calendar day as the previous commit
#   (with a strictly later time on that day), OR +[1..7] days ahead.
# - Choice is a true 50/50 using /dev/urandom (fallbacks provided).
# - Time of day is randomized (HH:MM:SS), always forward.
# - Uses UTC for simplicity and consistency across systems.
#
# After writing the new script, this script commits the change using the new
# qs_commit.sh so the fix itself follows the rule.
#
# Usage:
#   bash scripts/patch_commit_script_fix.sh

set -euo pipefail

mkdir -p scripts

cat > scripts/qs_commit.sh <<'QSCOMMIT'
#!/usr/bin/env bash
...
chmod +x scripts/qs_commit.sh

git add scripts/qs_commit.sh
# Use the freshly written script to commit itself under the fair dating rule
scripts/qs_commit.sh "chore(commit): fair 50/50 dating via /dev/urandom; ensure forward times; UTC-safe" --all

echo "✅ Fixed qs_commit.sh and committed the change using the new dating policy."
