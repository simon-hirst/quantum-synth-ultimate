...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setPauseLabel = () => {
    btnPause.textContent = viz.isPaused()
      ? "Resume rotation (P)"
      : "Pause rotation (P)";
  };
  ...
  btnStart.onclick = async () => {
    ...
    try {
      ...
      statusEl.textContent =
        "Screen sharing active ✓ — Best compatibility: choose “Entire Screen” + “Share audio”";
    } catch (e: any) {
      console.error(e);
      statusEl.textContent =
        "Screen share failed. Tip: select “Entire Screen” and enable “Share audio”";
    }
  };
  btnStop.onclick = () => {
    viz.stopScreenShare();
  ...
  {
    const shellEl = shell;
    const btnToggleSide = document.getElementById(
      "btnToggleSide",
    ...
    if (btnFullscreen) {
      btnFullscreen.addEventListener("click", async () => {
        const target: any =
          document.querySelector(".qs-main") || document.documentElement;
        const fsOn = !!(
          document.fullscreenElement ||
          (document as any).webkitFullscreenElement ||
          (document as any).msFullscreenElement
        );
        if (!fsOn) await requestFs(target);
        else await exitFs();
      });
    }
  }
...
(() => {
  try {
    ...
    let fsb = document.getElementById(
      "btnFullscreen",
    ) as HTMLButtonElement | null;
    if (!fsb) {
    ...
    console.warn("QS009 injection failed", e);
  }
})();
